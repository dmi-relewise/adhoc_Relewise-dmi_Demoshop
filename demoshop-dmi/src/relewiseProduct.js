...
export async function createProducts() {
    ...
    loadData.forEach(element => {
        ...
        const categories = element.category.split(">").map(function (item) { return { id: item, displayName: [{ language: "en", value: item }] }; });

        product.categoryPaths(b => b.path(p => { categories.forEach(category => { p.category(category); }); }));
